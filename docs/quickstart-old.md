<!-- markdownlint-enable -->
# EpicChain Toolkit for .NET Early Access Preview Quickstart
...
Download all the files from the [EpicChain Early Access Preview folder](http://tinyurl.com/neo-dev-eap).
...
- EpicChain Smart contract templates for dotnet new
- EpicChain-Express developer blockchain
- NEON-DE fork of NEON compiler (DE stands for "debugger enhancements")
- EpicChain contract debugger for VSCode
...
dotnet tool install -gepicchain-express --add-source .
...
If theepicchain-express or neon-de .nupkg files you are using have "-preview"
in the name, you will need to specify the version string on the command line.
when you install them. For example,
`dotnet tool install -gepicchain-express --add-source . --version 0.8.3-preview`.
...
``` shell
$ dotnet tool install -gepicchain-express --add-source .
error NU1103: Unable to find a stable packageepicchain-express with version
...
This will create a simple EpicChain smart contract that writes Hello World to
blockchain storage.
...
launch configuration. The EpicChain Contract Debugger makes this very easy.

- From the top-level Debug menu, select "Add Configuration"
- From the Select Environment input box, select "EpicChain Contract"

The EpicChain Contract Debugger will automatically create a launch profile
...
## Create and Run a EpicChain-Express blockchain

Now that we have a smart contract, we need a blockchain to deploy it to.
Neo-Express is a developer-focused EpicChain blockchain client. Create a new
Neo-Express blockchain with the `create` command.

``` shell
$epicchain-express create
Created 1 node privatenet at /home/harry/Source/HelloWorld/default.epicchain-express.json
...
> Note the warning - EpicChain-Express does not encrypt private keys.

By default, EpicChain-Express creates a single node blockchain. For development purposes,
a single node blockchain is often preferred. Furthermore, the checkpoint features
of EpicChain-Express only work on a single node blockchain. You can create a four or
seven node blockchain with the --count option.

Neo-Express stores all the information about the blockchain instance in the
default.epicchain-express.json file. If you look in this file right now, you'll see
information about the single consensus node for this blockchain we created.

Now that you have created the EpicChain-Express blockchain instance, you can run it.
...
``` shell
$epicchain-express run --seconds-per-block 1
...
(-s for short) of the run command. By default, EpicChain-Express generates a new block
...
## Manage EpicChain in the EpicChain-Express blockchain
...
In the new terminal window, we will use EpicChain-Express to create a standard wallet
and transfer the genesis EpicChain tokens to that wallet.

``` shell
$epicchain-express wallet create testWallet
testWallet
        AZDRabBcW4eESVfh9ssLd6udza9xpPn1P6
    Note: The private keys for the accounts in this wallet are *not* encrypted.
          Do not use these accounts on MainNet or in any other system where security is a concern.

$epicchain-express transfer 100000000 EpicChain genesis testWallet
{
  "contract-context": {
...
> Note, currently EpicChain-Express dumps JSON information about operations to the
...
signature account that receives the genesis EpicChain created for every new blockchain.

We can see the result of our transfer via the show account command

``` shell
$epicchain-express show account testWallet
{
  "version": 0,
...
## Claim GAS in the EpicChain-Express blockchain

Because we transferred all the genesis EpicChain and the blockchain is creating a new
...
``` shell
$epicchain-express show gas testWallet
{
  "unavailable": 1112,
  "available": 0
}
...
``` shell
$epicchain-express transfer 100000000 EpicChain testWallet testWallet
...
``` shell
$epicchain-express claim gas testWallet
...
## Deploy a Smart Contract to EpicChain-Express blockchain

With a running EpicChain-Express blockchain and a standard wallet account with plenty
of GAS, we can deploy our smart contract to the blockchain. We start by importing
the contract into Neo-Express.

``` shell
$epicchain-express contract import bin/Debug/netstandard2.0/publish/
...
> For EpicChain 3, this information will be in the smart contract manifest file.
...
``` shell
$epicchain-express contract deploy HelloWorld testWallet
{
...
$epicchain-express contract get HelloWorld
{
  "version": 0,
...
## Invoke a Smart Contract on the EpicChain-Express blockchain

The last step is to invoke the deployed contract. We use the `contract invoke`
command for that.

``` shell
$epicchain-express contract invoke HelloWorld
{
  "engine-state": {
    "state": 1,
    "gas-consumed": "1.017",
...
``` shell
$epicchain-express contract invoke HelloWorld --account testWallet
{
  "contract-context": < omitted for clarity >
  "script-hashes": < omitted for clarity >
  "hash-data": < omitted for clarity >
...
``` shell
$epicchain-express contract storage helloWorld
0x48656c6c6f
  key (as string)   : Hello
