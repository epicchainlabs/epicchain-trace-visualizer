...
//
// TraceDebugTests.cs file belongs toepicchain-express project and is free
...
namespace test.bctklib
{
    public class TraceDebugTests
    {
        ...
        public void can_deserialize_trace_record_with_no_gas()
        {
            ...
            TraceRecord_WriteWithoutGas(writer, options, VMState.BREAK, Array.Empty<EpicChain.VM.ExecutionContext>(), _ => UInt160.Zero);
        ...
        static void TraceRecord_WriteWithoutGas(
           ...
           IReadOnlyCollection<EpicChain.VM.ExecutionContext> contexts, Func<EpicChain.VM.ExecutionContext, UInt160> getScriptIdentifier)
        {
        ...
        {
            var writer = new ArrayBufferWriter<byte>();
            TraceRecord.Write(writer, options, VMState.BREAK, 1000, Array.Empty<EpicChain.VM.ExecutionContext>(), _ => UInt160.Zero);
